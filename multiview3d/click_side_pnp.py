...
Usage:  python click_side_pnp.py 4 --tag charucoall_20260821_172921
...
ROOTS = {4: r"D:\h5_rec_cam4", 5: r"C:\h5_rec_cam5"}
FOLDERS: dict = {}  # filled from --tag in main()
F_NOMINAL = {4: 6000.0 / 5.5, 5: 6000.0 / 5.5}

# Which board end faces the side camera depends on how the strip was laid
# down THIS time. Check which columns the adjacent top cam detects:
# low columns (0..~10) at your end -> --end low; high (~34..43) -> --end high.
def points_for(end: str, ncols: int = 3):
    ks = range(1, ncols + 1)
    if end == "high":
        xs = [180.0 - SQ * k for k in ks]             # 176, 172, 168, ... mm
    else:
        xs = [SQ * k for k in ks]                     # 4, 8, 12, ... mm
    pts = []
...
def main() -> None:
    ...
    parser.add_argument("cam", type=int, choices=(4, 5))
    parser.add_argument("--tag", required=True,
                        help="capture tag, e.g. charucofin_20260825_141324")
    parser.add_argument("--end", required=True, choices=("low", "high"),
                        help="which strip end faces this camera: check the "
                             "adjacent top cam's detected columns (0..10 -> low, "
                             "34..43 -> high)")
    parser.add_argument("--ncols", type=int, default=3,
                        help="how many columns in from the near end to click "
                             "(2 points each; default 3 = 6 clicks; more = "
                             "stronger pose, click as many as you can see)")
    args = parser.parse_args()
    cam = args.cam
    FOLDERS[cam] = os.path.join(ROOTS[cam], f"{args.tag}_cam{cam}_png")
    img = load_mean(cam)
    vis = cv2.cvtColor(img, cv2.COLOR_GRAY2BGR)
    pts = points_for(args.end, args.ncols)
    clicked = []
    state = {"i": 0}
    ...
    def on_mouse(event, x, y, flags, param):
        if event == cv2.EVENT_LBUTTONDOWN and state["i"] < len(pts):
            ...
            cv2.putText(vis, str(state["i"] + 1), (x + 4, y - 4),
    ...
    while True:
        ...
        if key == ord("z") and clicked:
            ...
            for j, c in enumerate(clicked):
                ...
                cv2.putText(vis, str(j + 1), (p[0] + 4, p[1] - 4),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.2, (0, 0, 255), 1)
            prompt()
